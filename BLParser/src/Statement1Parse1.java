...
public final class Statement1Parse1 extends Statement1 {
    ...
    private static void parseIf(Queue<String> tokens, Statement s) {
        ...
        Statement ifBlock = new Statement1();
        Statement elseBlock = new Statement1();
        boolean ifElse = false;
        Reporter.assertElseFatalError(tokens.dequeue().equals("IF"),
                "Violation of: If statement begins with IF");

        Condition condition = parseCondition(tokens.dequeue());

        Reporter.assertElseFatalError(tokens.dequeue().equals("THEN"),
                "Violation of: body after IF statement begins with THEN");

        ifBlock.parseBlock(tokens);

        ifElse = tokens.front().equals("ELSE");
        if (ifElse) {

            Reporter.assertElseFatalError(tokens.dequeue().equals("ELSE"),
                    "Violation of: Else statement begins with ELSE");

            elseBlock.parseBlock(tokens);
        }

        Reporter.assertElseFatalError(tokens.dequeue().equals("END"),
                "Violation of: statement after if block is END");
        Reporter.assertElseFatalError(tokens.dequeue().equals("IF"),
                "Violation of: statement after if block is IF");
        if (ifElse) {
    ...
    private static void parseWhile(Queue<String> tokens, Statement s) {
        ...
        Reporter.assertElseFatalError(tokens.dequeue().equals("WHILE"),
                "Violation of: While statement begins with WHILE");

        Condition condition = parseCondition(tokens.dequeue());
        Reporter.assertElseFatalError(tokens.dequeue().equals("DO"),
                "Violation of: body after WHILE statement begins with DO");

        Statement whileBlock = new Statement1Parse1();
        whileBlock.parseBlock(tokens);

        Reporter.assertElseFatalError(tokens.dequeue().equals("END"),
                "Violation of: statement after while block is END");
        Reporter.assertElseFatalError(tokens.dequeue().equals("WHILE"),
                "Violation of: statement after while block is WHILE");
    ...
    public void parse(Queue<String> tokens) {
        ...
        Reporter.assertElseFatalError(
                tokens.front().equals("IF") || tokens.front().equals("WHILE")
                        || Tokenizer.isIdentifier(tokens.front()),
                "Error: Expect a If, While, or Identifier, but found: "
                        + tokens.front());
